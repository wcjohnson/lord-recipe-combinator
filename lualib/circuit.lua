...
local function cache_qualities()
	...
	for name, p in pairs(prototypes.quality) do
		if name ~= "quality-unknown" then table.insert(qps, p) end
	end
...
function Builder:constant_combi(signals, description, name)
	...
	for i = 1, #signals do
		...
		filters[j] = {
			value = util.merge({ { comparator = "=", quality = normal }, sig[1] }),
			min = sig[2],
		}
	end
	section.filters = filters
...
function Builder:make_combi(args)
	...
	if args.arithmetic then
		name = args.combinator_name
			...
			or "recipe-combinator-component-arithmetic-combinator"
	elseif args.selector then
		name = args.combinator_name
			or (args.visible and "selector-combinator")
			or "recipe-combinator-component-selector-combinator"
	else
		name = args.combinator_name
			or (args.visible and "decider-combinator")
			or "recipe-combinator-component-decider-combinator"
	end
	...
	local ret = self.surface.create_entity({
		...
		position = self.position,
		force = self.force,
		direction = args.orientation or defines.direction.north,
...
function Builder:decider(args)
	...
	for i, clause in ipairs(args.output or { args }) do
		local output = {
			networks = clause.WO or NBOTH,
			copy_count_from_input = not clause.set_one,
			signal = clause.out or EACH,
			constant = clause.constant or 1,
		}
		if i > 1 then
...
local function build_flag_matrix(builder, input, rows, prefix)
	...
	for _idx, row in ipairs(rows) do
		...
		for _, colsig in ipairs(cols) do
			...
			if not bitno[colsig_str] then
				...
				table.insert(
					meaning_layers[floor(nbits / 32) + 1],
					{ colsig, signed_lshift(1, nbits % 32) }
				)
				nbits = nbits + 1
			end
			...
			if bitses[grp] then
				bitses[grp] = bitses[grp] + signed_lshift(1, bit % 32)
			else
				bitses[grp] = signed_lshift(1, bit % 32)
			end
		end
	...
	for layer = 1, ceil(nbits / 32) do
		local flag_combi = builder:constant_combi(
			flag_layers[layer],
			prefix .. "flag" .. tostring(layer)
		)
		local flag_meaning = builder:constant_combi(
			meaning_layers[layer],
			prefix .. "flag_meaning" .. tostring(layer)
		)
...
local function cache_modules()
	...
	for _name, module in
		pairs(
			prototypes.get_item_filtered({ { filter = "type", type = "module" } })
		)
	do
		table.insert(g_all_modules, module.name)
		if
			not module_category_to_module[module.category]
...
local function cache_spoilage() g_spoilage_cache = {} end
...
local function build_sparse_matrix(args)
	...
	for _idx, row in ipairs(rows) do
		...
		for layer, col in ipairs(combo) do
			...
			if multiply_by_input and divider ~= 1 then
				dividers_not_all_one[layer] = true
			end
	...
	round_up = (not any_dividers_all_one) and round_up
	...
	if not flags_only and any_dividers_all_one then
		buf_input = builder:arithmetic({
			L = EACH,
			op = "+",
			R = 0,
			description = prefix .. "buf",
			green = { input },
		})
	end
	local first_output = nil
	for layer = 1, #idx_data do
		local idx_combi = builder:constant_combi(
			idx_data[layer],
			prefix .. "idx" .. tostring(layer)
		)
		local jdx_combi = builder:constant_combi(
			jdx_data[layer],
			prefix .. "jdx" .. tostring(layer)
		)
		...
		if
			dividers_not_all_one[layer]
			and signal_arrays_equal(divider_data[layer], dividers_prev)
		then
			-- ... preferably from the cache
			my_buf_input, mod_input = prev_div_input, prev_mod_input
		elseif dividers_not_all_one[layer] then
			local dividends = builder:constant_combi(
				divider_data[layer],
				prefix .. "divctx" .. tostring(layer)
			)
			my_buf_input = builder:arithmetic({
				NL = NGREEN,
				L = EACH,
				op = "/",
				NR = NRED,
				R = EACH,
				description = prefix .. "div" .. tostring(layer),
				green = { input },
				red = { dividends },
			})
			if round_up then
				mod_input = builder:arithmetic({
					NL = NGREEN,
					L = EACH,
					op = "%",
					NR = NRED,
					R = EACH,
					description = prefix .. "mod" .. tostring(layer),
					green = { input },
					red = { dividends },
				})
			end
			prev_div_input, prev_mod_input, dividers_prev =
				my_buf_input, mod_input, divider_data
		else
		...
		if flags_only then
			...
			local ent_combi = builder:constant_combi(
				entry_data[layer],
				prefix .. "ent" .. tostring(layer)
			)
			local dotp
			if multiply_by_input then
				...
				if mod_input then
					...
					rucomb
						.get_wire_connector(ORED, true)
						.connect_to(dotp.get_wire_connector(ORED, true))
		...
		if first_output then
			first_output
				.get_wire_connector(OGREEN, true)
				.connect_to(this_output.get_wire_connector(OGREEN, true))
			first_output
				.get_wire_connector(ORED, true)
				.connect_to(this_output.get_wire_connector(ORED, true))
		else
...
local function find_children_in(surface, bbox)
	...
	tlib.filter_in_place(
		children,
		function(child)
			return string.find(child.name, "^recipe%-combinator%-component%-")
		end
	)
	return children
end
...
local function destroy_components(entity)
	destroy_components_in(entity.surface, entity.bounding_box)
end
...
local function build_matrix_combinator(
	...
	local input_buffer = builder:arithmetic({
		L = EACH,
		op = "+",
		R = 0,
		description = "ri.input_buffer",
	})
	input_buffer
		.get_wire_connector(IGREEN, true)
		.connect_to(entity.get_wire_connector(IGREEN, true))
	input_buffer
		.get_wire_connector(IRED, true)
		.connect_to(entity.get_wire_connector(IRED, true))
	...
	if HAVE_QUALITY then
		...
		for idx, qual in ipairs(g_all_qualities) do
			table.insert(
				literal_quality,
				{ util.merge({ QUALITY, { quality = qual } }), idx }
			)
			...
			table.insert(
				decisions_getqual,
				{ NL = NGREEN, L = ANYTHING, op = "=", R = idx }
			)
			table.insert(
				decisions_getqual,
				{ and_ = true, NL = NRED, L = EACH, op = "=", R = idx }
			)
		end
		...
		end_of_input_stage = builder:selector({
			op = "quality-transfer",
			qual = g_all_qualities[0],
			out = EACH,
			green = { select_one },
			description = "ri.set_normal_qual",
		})
		...
		end_of_input_stage_normal_only = builder:selector({
			...
			qual = g_all_qualities[0],
			red = { select_one },
			description = "ri.check_normal_qual",
		})
		...
		quality_buffer = builder:decider({
			...
			output = { { out = EACH } },
			description = "ri.getqual",
			green = { select_one_getqual },
			red = { quality_list },
			-- visible=true
		})
	else
		...
		end_of_input_stage = builder:arithmetic({
			L = EACH,
			op = "+",
			R = 0,
			description = "ri.buffer3",
			green = { select_one },
		})
		selected_signal = end_of_input_stage
	end
	...
	local function connect_output(combi, flags)
		...
		if band(flags, FLAG_RED) > 0 then
			combi
				.get_wire_connector(ORED, true)
				.connect_to(entity.get_wire_connector(ORED, true))
		end
		if band(flags, FLAG_GREEN) > 0 then
			combi
				.get_wire_connector(OGREEN, true)
				.connect_to(entity.get_wire_connector(OGREEN, true))
		end
	end
	...
	for flags, matrix in pairs(matrices_by_flags) do
		...
		if band(flags, FLAG_NOQUAL) > 0 then use_qual = nil end
		if band(flags, FLAG_DENSE) > 0 then
			...
			connect_output(
				build_flag_matrix(
					...
					input,
					matrix,
					"ri.matrix.flags" .. tonumber(flags) .. "."
				),
				flags
			)
			...
			connect_output(
				build_sparse_matrix({
					...
					input = input,
					rows = matrix,
					prefix = "ri.matrix.flags" .. tonumber(flags) .. ".",
					multiply_by_input = band(flags, FLAG_MULTIPLY) > 0,
					use_qual = use_qual,
					round_up = true, -- TODO
				}),
				flags
	...
	if
		HAVE_QUALITY
		and output_quality_sig
		and output_quality_flags
		and output_quality_flags > 0
	then
		local qs4
		if
			output_quality_sig.name == QUALITY.name
			and output_quality_sig.type == QUALITY.type
		then
			qs4 = builder:decider({
			...
			local quality_comb = builder:constant_combi(
				{ { output_quality_sig, 1 } },
				"ri.matrix.quality_cc"
			)
			qs4 = builder:selector({
				op = "quality-transfer",
				qual = QUALITY,
				out = output_quality_sig,
				red = { quality_buffer },
				green = { quality_comb },
				description = "ri.matrix.quality_buffer_4",
			})
		end
		local qs5 = builder:arithmetic({
			L = EACH,
			op = "+",
			R = 0,
			green = { qs4 },
			description = "ri.matrix.quality_buffer_5",
		})
		local qs6 = builder:arithmetic({
			L = EACH,
			op = "+",
			R = 0,
			green = { qs5 },
			description = "ri.matrix.quality_buffer_6",
		})
	...
	if output_all_inputs and output_all_inputs > 0 then
		...
		if HAVE_QUALITY and not output_all_inputs_quality then
			prev = builder:selector({
				op = "quality-transfer",
				qual = g_all_qualities[1],
				out = EACH,
				green = { prev },
				description = "ri.matrix.oai_quality",
			})
		else
			prev = builder:arithmetic({
				L = EACH,
				op = "+",
				R = 0,
				green = { prev },
				description = "ri.matrix.input.oai_buffer1",
			})
		end
		...
		for i = 3, 6 do
			prev = builder:arithmetic({
				...
				R = 0,
				green = { prev },
				description = "ri.matrix.input.oai_buffer" .. tostring(i),
			})
		end
	...
	if
		(output_selected_signal and output_selected_signal > 0)
		or (
			output_quantity_sig
			and output_quantity_flags
			and output_quantity_flags > 0
		)
	then
		local sel4 = builder:arithmetic({
			L = EACH,
			op = "+",
			R = 0,
			green = { selected_signal },
			description = "ri.matrix.input_buffer_4",
		})
		local sel5 = builder:arithmetic({
			L = EACH,
			op = "+",
			R = 0,
			green = { sel4 },
			description = "ri.matrix.input_buffer_5",
		})
		if
			output_quantity_sig
			and output_quantity_flags
			and output_quantity_flags > 0
		then
			local seln = builder:arithmetic({
				L = EACH,
				op = "+",
				R = 0,
				out = output_quantity_sig,
				green = { sel5 },
		...
		if output_selected_signal and output_selected_signal > 0 then
			local sel6 = builder:arithmetic({
				L = EACH,
				op = "+",
				R = 0,
				green = { sel5 },
				description = "ri.matrix.input_buffer_6",
			})
...
local function build_recipe_info_combinator(args)
	...
	local output_all_inputs_quality = HAVE_QUALITY
		and args.output_all_inputs_quality
	...
	local module_table = output_allowed_modules
			and ((one_module_per_category and g_modules_per_category) or g_all_modules)
		or {}
	...
	for _, machine in ipairs(machines) do
		...
		if machine_proto then -- in case you are eg pasting blueprints between mods, and the machine doesn't exist
			...
			if item_to_place and n_indicator < 4 then
				...
				indicator.set_filter(n_indicator, {
			...
			for cat, _ in pairs(machine_proto.crafting_categories) do
				if not crafting_time_scale[cat] then
					crafting_time_scale[cat] = absolute_time_scale
						/ machine_proto.get_crafting_speed(quality)
	...
	for category, _ in pairs(crafting_time_scale) do
		...
		local machine_has_modules = machine_proto.module_inventory_size
			and (machine_proto.module_inventory_size > 0)
		local recipes, is_spoilage

		recipes = prototypes.get_recipe_filtered({
		...
		for name, recipe in pairs(recipes) do
			...
			if suitable then
				local sig = is_spoilage
						and { type = "item", name = recipe.ingredients[1].name }
				...
				if output_recipe_ingredients then
					for idx = 1, #ingredients do
						...
						local fluid = (ingredient.type == "fluid") and FLAG_NOQUAL
							or flag_all_fluid
						row:set_entry(
				...
				if output_recipe_products then
					for idx = 1, #products do
						...
						local amt = product.amount
							or (product.amount_min + product.amount_max) / 2
						local fluid = (product.type == "fluid") and FLAG_NOQUAL
							or flag_all_fluid
						row:set_entry(
				...
				if output_recipe_count and output_recipe_count_sig then
					row:set_entry(
						output_recipe_count_sig,
						1,
						bor(
							output_recipe_count,
							FLAG_MULTIPLY + FLAG_NOQUAL + flag_all_fluid
						)
					)
				end
				...
				if output_crafting_machine and category_to_machine[category] then
					row:set_entry(
						{ type = "item", name = category_to_machine[category] },
						1,
						bor(
				...
				if output_allowed_modules and machine_has_modules then
					for idx = 1, #module_table do
						...
						local ok = (
							not recipe.allowed_module_categories
							or recipe.allowed_module_categories[cat]
						)
						ok = ok
							and (
								not machine_proto.allowed_module_categories
				...
				if input_item_ingredient or input_fluid_ingredient then
					for idx = 1, #ingredients do
						...
						if
							(input_item_ingredient and not is_fluid)
							or (input_fluid_ingredient and is_fluid)
						then
							index_by_item(
								row,
								ingredients[idx],
				...
				if input_item_product or input_fluid_product then
					for idx = 1, #products do
						...
						local amt = product.amount
							or (product.amount_min + product.amount_max) / 2
						if
							(input_item_product and not is_fluid)
							or (input_fluid_product and is_fluid)
						then
...
local function rollup_state_to_build_args(entity, rollup)
	...
	local ret = {
		...
		input_item_ingredient = ru.input_ingredient_group
			and ru.input_item_ingredient,
		input_fluid_ingredient = ru.input_ingredient_group
			and ru.input_fluid_ingredient,
		...
		output_quality = rollup_flags(
			ru.show_quality,
			false,
			false,
			ru.show_quality_red,
			ru.show_quality_green
		),
		output_selected = rollup_flags(
			ru.show_selected,
			false,
			false,
			ru.show_selected_red,
			ru.show_selected_green
		),

		output_allowed_modules = rollup_flags(
			ru.show_modules,
			false,
			false,
			rollup.show_modules_red,
			ru.show_modules_green
		),
		output_allowed_modules = rollup_flags(
			ru.show_modules,
			false,
		...
		output_recipe_ingredients = rollup_flags(
			ru.show_ingredients,
			ru.show_ingredients_ti,
			ru.show_ingredients_neg,
			rollup.show_ingredients_red,
			ru.show_ingredients_green
		),
		output_recipe_products = rollup_flags(
			ru.show_products,
			ru.show_products_ti,
			ru.show_products_neg,
			rollup.show_products_red,
			ru.show_products_green
		),
		output_recipe = rollup_flags(
			ru.show_recipe,
			ru.show_recipe_ti,
			ru.show_recipe_neg,
			rollup.show_recipe_red,
			ru.show_recipe_green
		),
		output_all_recipes = rollup_flags(
			ru.show_all_recipes,
			ru.show_all_recipes_ti,
			ru.show_all_recipes_neg,
			rollup.show_all_recipes_red,
			ru.show_all_recipes_green
		),
		output_crafting_machine = rollup_flags(
			ru.show_machines,
			false,
			false,
			rollup.show_machines_red,
			ru.show_machines_green
		),
		output_crafting_time = rollup_flags(
			ru.show_time,
			ru.show_time_ti,
			ru.show_time_neg,
			ru.show_time_red,
		...
		output_recipe_count = rollup_flags(
			ru.show_rcount,
			true,
			false,
			ru.show_rcount_red,
			ru.show_rcount_green
		),
		output_all_inputs = rollup_flags(
			ru.show_all_valid_inputs,
			false,
			false,
			ru.show_all_valid_inputs_red,
			ru.show_all_valid_inputs_green
		),
		output_quantity = rollup_flags(
			ru.show_quantity,
			false,
			false,
			ru.show_quantity_red,
			ru.show_quantity_green
		),
...
local function rebuild_combinator(combinator)
	local _, tags = remote.call("things-tags-v1", "get_tags", combinator)
	build_recipe_info_combinator(rollup_state_to_build_args(combinator, tags))
	local children = find_children_in(combinator.surface, combinator.bounding_box)
	strace.trace(
		"lord-recipe-combinator: rebuild_combinator: created",
		#children,
		"children for",
		combinator
	)
	remote.call("things", "set_transient_data", combinator, "children", children)
end
...
M.destroy_components_in = destroy_components_in
